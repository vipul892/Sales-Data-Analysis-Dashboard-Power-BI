# Sales-Data-Analysis-Dashboard-Power-BI <br>
📊 Sales Dashboard (2024–25) <br>
📌 Overview <br>
This project presents an interactive Sales Analysis Dashboard built in Power BI to monitor business performance for the years 2024–25.
The dashboard provides insights into sales, profit, quantity, brand performance, and state-wise distribution to support data-driven decision-making.

🛠 Tools Used :- <br>
* Power BI Desktop <br>
* Power Query <br>
* DAX <br>
* Excel / CSV Dataset <br>

📊 Key Metrics:-
* Total Sales: 15M <br>
* Total Cost: 12M <br>
* Total Profit: 4M <br>
* Total Quantity: 1953 <br>
* Total Transactions: 791 <br>

📈 Dashboard Highlights:- <br>
* Quantity by Product Category <br>
* Brand-wise Sales Distribution <br>
* State-wise Sales Map (India) <br>
* Supervisor-level Performance Filter <br>
* Interactive KPIs and Visualizations <br>

🚀 How to Use :- <br>
* Download the .pbix file <br>
* Open in Power BI Desktop <br>
* Use filters and slicers to explore insights <br>

📸 Preview

![Sales Dashboard](sales_data_analysis_dashbord.png)

👨‍💻 Author <br>
Vipul Alsundkar <br>
Data Analyst | SQL | Excel | Power BI <br>
📍 Pune, Maharashtra <br>
🔗 LinkedIn: https://linkedin.com/in/vipul-alsundkar-6891593a0 <br>
🔗 GitHub: https://github.com/vipul892 <br>
